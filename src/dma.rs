...
impl Dma {
    ...
    pub fn configure_request(&mut self, channel: u8, request: DmaRequest) {
    ...
    pub fn configure_transfer(
        ...
        assert!(channel < 8, "Channel must be 0-7");
        assert!(xfer_count > 0, "Transfer count must be > 0");
    ...
    /// LDMA_SWREQ register.
    pub fn start_transfer(&mut self, channel: u8) {
        assert!(channel < 8, "Channel must be 0-7");
    ...
    pub fn is_transfer_done(&self, channel: u8) -> bool {
        assert!(channel < 8, "Channel must be 0-7");
    ...
    pub fn clear_done_flag(&mut self, channel: u8) {
        assert!(channel < 8, "Channel must be 0-7");
