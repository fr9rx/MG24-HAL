...
#define GPIO_P_DOUT_SET(port, value) ((GPIO)->P_SET[port].DOUT = (value))
#define GPIO_P_DOUT_CLR(port, value) ((GPIO)->P_CLR[port].DOUT = (value))
#define GPIO_P_DOUT_TGL(port, value) ((GPIO)->P_TGL[port].DOUT = (value))
#else
#define GPIO_P_DOUT_SET(port, value) ((GPIO)->P[port].DOUT |= (value))
#define GPIO_P_DOUT_CLR(port, value) ((GPIO)->P[port].DOUT &= ~(value))
#define GPIO_P_DOUT_TGL(port, value) ((GPIO)->P[port].DOUT ^= (value))
#endif
...
typedef enum
{
  /** Default drive mode (fallback). */
#if defined(GPIO_P_CTRL_DRIVEMODE_STANDARD)
  gpioDriveModeStandard = GPIO_P_CTRL_DRIVEMODE_STANDARD,
#else
  gpioDriveModeStandard = 0,
#endif
#if defined(GPIO_P_CTRL_DRIVEMODE_LOWEST)
  gpioDriveModeLowest   = GPIO_P_CTRL_DRIVEMODE_LOWEST,
#else
  gpioDriveModeLowest   = 0,
#endif
#if defined(GPIO_P_CTRL_DRIVEMODE_HIGH)
  gpioDriveModeHigh     = GPIO_P_CTRL_DRIVEMODE_HIGH,
#else
  gpioDriveModeHigh     = 0,
#endif
#if defined(GPIO_P_CTRL_DRIVEMODE_LOW)
...
typedef enum
{
  ...
  gpioModePushPullDrive
    ...
    = _GPIO_P_MODEL_MODE0_PUSHPULLDRIVE,
#else
    = gpioModePushPull,
#endif
  /** Wired-or output */
...
#if defined(_GPIO_P_MODEL_MODE0_WIREDANDDRIVEPULLUP)
...
#if defined(_GPIO_P_MODEL_MODE0_WIREDANDDRIVEPULLUPFILTER)
